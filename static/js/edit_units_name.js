document.addEventListener('DOMContentLoaded', function() {
    const form = document.getElementById('edit-unit-form');
    const errorMessage = document.getElementById('error-message');
    const unitId = document.getElementById('units_id').value;
    const csrfToken = document.querySelector('[name=csrfmiddlewaretoken]').value;

    form.addEventListener('submit', function(e) {
        e.preventDefault();

        const unitName = document.getElementById('unit_name').value.trim();

        errorMessage.style.display = 'none';

        if (!unitName) {
            errorMessage.textContent = 'Unit Name and Code are required.';
            errorMessage.style.display = 'block';
            return;
        }

        fetch(`/commodities/units/update/${unitId}/`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRFToken': csrfToken,
            },
            body: JSON.stringify({
                unit_name: unitName,
            }),
        })
        .then(response => response.json())
        .then(data => {
            if (data.status === 'success') {
                Swal.fire({
                    icon: 'success',
                    title: 'Success!',
                    text: 'Units updated successfully!',
                    confirmButtonText: 'OK',
                    allowOutsideClick: false,     // Prevent click outside
                    allowEscapeKey: false,        // Prevent ESC key
                    allowEnterKey: true           // Optional: allow Enter to confirm
                }).then((result) => {
                    if (result.isConfirmed) {
                        window.location.href = '/commodities/units/type/';
                    }
                });
            } else {
                errorMessage.textContent = data.message || 'Something went wrong.';
                errorMessage.style.display = 'block';
            }
        })
        .catch(error => {
            errorMessage.textContent = 'Server Error. Please try again later.';
            errorMessage.style.display = 'block';
        });
    });
});
